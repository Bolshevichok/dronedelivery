...
func InitMissionCreatedConsumer(cfg *config.Config, missionProcessor *missionprocessor.MissionProcessorImpl) *mission_created_consumer.MissionCreatedConsumerImpl {
...
func InitTelemetryConsumer(cfg *config.Config, telemetryProcessor *telemetryprocessor.TelemetryProcessorImpl) *telemetry_consumer.TelemetryConsumerImpl {
	kafkaBrokers := []string{fmt.Sprintf("%s:%d", cfg.Kafka.Host, cfg.Kafka.Port)}
